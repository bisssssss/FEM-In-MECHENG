...
void writeSolutionsToFileCA3_1(const Vector<double>& U, std::string solutionTag){
  hid_t file_id, dataset_id, dataspace_id;
  herr_t      status;
  hsize_t dimens_1d;
  std::cout << "writing solution for Coding Assignment 3, prob 1 to file : " << solutionTag.c_str() << ".h5" << std::endl;
...
void writeSolutionsToFileCA3_2(const Vector<double>& U, double torque, std::string solutionTag){
  hid_t file_id, dataset_id, dataspace_id;
  herr_t      status;
  hsize_t dimens_1d;
  std::cout << "writing solution for Coding Assignment 3, prob 2 to file : " << solutionTag.c_str() << ".h5" << std::endl;
 ...
 //create HDF5 file
  ...
  dimens_1d = 1;
  double scalarValue[1]={torque};
  dataspace_id = H5Screate_simple(1, &dimens_1d, NULL);
  dataset_id = H5Dcreate(file_id, "torque", H5T_IEEE_F64LE, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
