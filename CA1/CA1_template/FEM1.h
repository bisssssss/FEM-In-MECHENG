...
class FEM
{
 public:
  ...
  //std::vector<vector<double>> quad_Nmap; //
  //void gen_quad_Nmap();
  unsigned int          noes;
  ...
  double                basisFunctionOrder, L, g1, g2;    
  double                EE, Area, f_bar, f_hat, h_in;
  unsigned int          prob;
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  //EDIT
  //std::cout << "node " << node << " xi " << xi << std::endl;
  double LIdiv = 0.;
  double LIden = 1.;
  for(unsigned int Ni = 0; Ni < basisFunctionOrder + 1; Ni ++){
  ...
  //std::cout << LIdiv << " " << LIden << " " << value << std::endl;
  value = LIdiv / LIden;
...
/*
template <int dim>
void FEM<dim>::gen_quad_Nmap() {

  quad_Nmap.resize(quadRule);

  for(int i = 0; i < quadRule; i++)
    quad_Nmap[i].resize(basisFunctionOrder + 1);

  for(int idxi = 0; idxi < quadRule; idxi++){
    double xii = quad_points[i];
    std::vector<double> diffv;
    for(int i = 0; i < basisFunctionOrder + 1; i++)
      diffv.push_back(xii - xi_at_node(triangulation[i]));

    for(int Ni = 0; Ni < basisFunctionOrder + 1; Ni++){
      doulbe LIdiv = 0
...
void FEM<dim>::setup_system(){
  ...
  EE = 1.e11; Area = 1.e-4; f_bar = 1.e11; f_hat = 1.e12; h_in = 1.e6;
  g1 = 0; g2 = 0.001; //EDIT
  ...
  quad_points[0] = -1./3. * sqrt(5. + 2. * sqrt(10. / 7.)); //EDIT
  quad_points[1] = -1./3. * sqrt(5. - 2. * sqrt(10. / 7.)); //EDIT
  quad_points[2] = 0.; //EDIT
  quad_points[3] = 1./3. * sqrt(5. - 2. * sqrt(10. / 7.)); //EDIT
  quad_points[4] = 1./3. * sqrt(5. + 2. * sqrt(10. / 7.));  //EDIT  


  quad_weight[0] = (322. - 13. * sqrt(70.)) / 900.; //EDIT
  quad_weight[1] = (322. + 13. * sqrt(70.)) / 900.; //EDIT
  quad_weight[2] = 128. / 225.; //EDIT
  quad_weight[3] = (322. + 13. * sqrt(70.)) / 900.; //EDIT
  quad_weight[4] = (322. - 13. * sqrt(70.)) / 900.; //EDIT
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
    ...
    std::cout << "local nodes " ;
    for (int dd = 0; dd < local_dof_indices.size(); dd++)
      std::cout << local_dof_indices[dd] << " ";
    std::cout << std::endl;
    ...
    if(nelem != 0){
      for(unsigned int i = 2; i < dofs_per_elem; i++){
      local2global[i] = local_dof_indices[i] - dofs_per_elem;
    }
    } 
    //x = h_total;
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      for(unsigned int q=0; q<quadRule; q++){
        ...
        //EDIT - Define Flocal.
        if(prob == 1 || prob == 2)
          Flocal[A] += quad_weight[q] * f_bar * x * 
                basis_function(A, quad_points[q]) * Area * h_e / 2;
        else if(prob == 3 || prob == 4)
          Flocal[A] += quad_weight[q] * f_hat * x * (L-x) * 
                basis_function(A, quad_points[q]) * Area * h_e / 2;
      }
    }
    double bk = Flocal[1];
    for(unsigned int i = 1; i < dofs_per_elem - 1; i++){
      Flocal[i] = Flocal[i+1];
    }
    Flocal[dofs_per_elem - 1] = bk;
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      ...
      for(unsigned int B=0; B<dofs_per_elem; B++){
        ...
        for(unsigned int q=0; q<quadRule; q++){
          ...
          LI += quad_weight[q] * basis_gradient(A, quad_points[q]) * basis_gradient(B, quad_points[q]);
        }
        Klocal[A][B] = LI * EE * Area / h_e * 2;
      }
    }
    //std::cout << x << std::endl;
    //std::cout << h_e << std::endl;
    std::cout << Flocal << std::endl;
    for(unsigned int ii = 0; ii < dofs_per_elem; ii++){
      for(unsigned int jj = 0; jj < dofs_per_elem; jj++){
        std::cout << Klocal[ii][jj] << " ";
      }
      std::cout << std::endl;
    }
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      ...
      F[nelem * (dofs_per_elem - 1) + A] += Flocal[A]; 
      for(unsigned int B=0; B<dofs_per_elem; B++){
  //      std::cout << B << " " << nelem * (dofs_per_elem - 1) + A << " " <<  nelem * (dofs_per_elem - 1) + B << std::endl;
//std::cout << B << " " << local_dof_indices[A] << " " <<  local_dof_indices[B] << std::endl;
        K.add(local_dof_indices[A] - (dofs_per_elem-1), local_dof_indices[B] - (dofs_per_elem-1), Klocal[A][B]);
      }

    }
    nelem ++;
            for(unsigned int ii = 0; ii < noes*basisFunctionOrder+1; ii++){
      for(unsigned int jj = 0; jj < noes*basisFunctionOrder+1; jj++){
        std::cout << K.el(ii,jj) << " ";
      }
      std::cout << std::endl;
    } 
...
void FEM<dim>::output_results (){
  ...
  std::string filename = "solution_p" + std::to_string(prob) + "_" + std::to_string(noes) + "_";
  if (basisFunctionOrder == 1) filename += "linear.vtk";
  else if (basisFunctionOrder == 2) filename += "quadratic.vtk";
  else if (basisFunctionOrder == 3) filename += "cubic.vtk";
